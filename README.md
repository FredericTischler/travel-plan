...
ansible/roles/   6 rôles de provisionnement (1 rôle = 1 responsabilité)
...
| `postgres` | Rend `compose.postgres.yml` (image épinglée, `data-net`, `mem_limit: 256m`, healthcheck, **aucun port publié**). Crée 2 bases (`identity_db`, `payment_db`) et 2 comptes `NOSUPERUSER/NOCREATEDB/NOCREATEROLE` propriétaires chacun de leur seule base. Révoque `CONNECT` à `PUBLIC` puis l'accorde au seul propriétaire → cloisonnement croisé effectif. Peut lire ses credentials depuis Vault (`postgres_vault_read`, `false` par défaut) et écrit le mot de passe superuser dans `/opt/travel-plan/.env`. Ne crée **aucune** table (Flyway côté services). |
| `neo4j` | Rend `compose.neo4j.yml` : image épinglée, `data-net`, `mem_limit: 1g`, volume nommé, healthcheck, aucun port publié. |
| `vault` | Rend `compose.vault.yml` (Vault **dev mode**, `data-net`, `mem_limit: 96m`, root token via variable d'env, aucun port publié). Vérifie le moteur KV v2 sur `secret/`, écrit 6 chemins de secrets runtime (`infra/postgres-superuser`, `identity/db`, `payment/db`, `payment/stripe`, `payment/paypal`, `travel/db`) et pose 3 policies cloisonnées (`identity-policy`, `payment-policy`, `travel-policy`), chacune limitée à son propre préfixe. |
...
| Rôle | Test |
|---|---|
| `postgres`, `neo4j`, `vault`, `traefik` | Scénario **Molecule** (driver docker, conteneurs frères) avec étape `idempotence` (2ᵉ converge = `changed=0`) et `verify` assertant l'état réel |
...
| Ansible | Molecule sur 4 rôles sur 6 (cf. tableau plus haut) |
...
### CI/CD
- **Jenkins : absent.** `ci/jenkins/` ne contient qu'un README marqué « Placeholder
  Phase 0 ». Aucun `Jenkinsfile`, aucun Dockerfile, aucune config, aucun rôle Ansible.
- **SonarQube : absent.** Même situation dans `ci/sonarqube/`.
- Aucun pipeline de build/test/déploiement automatisé n'existe, donc aucune exécution
  de tests par PR.

### Sécurité / secrets
- **`JWT_SIGNING_KEY` en clair dans le dépôt** : la valeur
  `dev-insecure-jwt-signing-key-change-me-...` est écrite en dur dans les fragments
  `docker-compose.identity.yml` et `docker-compose.payment.yml` (et doit y être
  identique). Elle n'est **pas** câblée à Vault, contrairement aux credentials DB.
  Dette explicitement assumée dans le code, pas un oubli — mais c'est un secret versionné.
- **Vault non consommé au runtime par les services** : les services ne parlent jamais
  à Vault. Seul le rôle `postgres` sait lire Vault, et uniquement si
  `postgres_vault_read=true` (défaut `false`). Les mots de passe applicatifs des
  fragments Compose retombent sur des placeholders `changeme_*`.
...
- `ansible/README.md`, `services/README.md` et `services/admin-dashboard/README.md`
  décrivent encore un état « Placeholder Phase 0 » obsolète. Les README de
